...
.env      ─→                 │
                             ...
                             │
                             └─ 收盘触发
                                 ├─ data_fetcher  ← 腾讯 sqt（五指数）+ qt（个股）
                                 ├─ llm_client    ← OpenAI 兼容（连接池）
                                 ├─ summarizer    → 结构化数据 + LLM 叙事
                                 └─ mail_sender   → 富文本 HTML 邮件
```

## 收盘总结格式

四板块邮件：大盘概况表 → 市场特征 → 自选股表（9列） → 后市展望

| 板块 | 数据来源 | 说明 |
|------|---------|------|
| 大盘概况表 | 腾讯 sqt | 五指数（上证/深证/创业板/科创50/北证50），成交额环比通过本地 JSON 持久化计算 |
| 市场特征 | LLM 生成 | 5-8 个要点，基于指数数据判断情绪/资金/结构/技术面 |
| 自选股表 | 腾讯 qt + K线 | 收盘价/涨跌幅/成交额/换手率/振幅/均线关系/走势特征 |
| 后市展望 | LLM 生成 | 3-4 句话短期趋势判断 |
...
| 数据 | 来源 | API | 请求量 |
|------|------|-----|--------|
| 个股实时行情 | 腾讯财经 | `qt.gtimg.cn` | 1 次/轮询（7 股合并）+ 1 次/收盘 |
| 个股历史 K 线 | 腾讯财经 | `web.ifzq.gtimg.cn` | 1 次/股（仅收盘） |
| 五大指数 | 腾讯财经 | `web.sqt.gtimg.cn` | 1 次/收盘（5 指数合并） |
| 指数环比 | 本地 JSON 缓存 | 文件读写 | 0 次 HTTP |
| 全市场成交额 | 上证+深证指数求和 | 已有数据 | 0 次额外请求 |

> **2026-07-23 更新**：去掉 `stock_zh_a_spot()` 拉取 5000+ 只股票，全市场成交额改用上证+深证指数成交额求和。大盘概况从 27 秒降到 < 1 秒，涨停跌停家数/中位数不再输出（指数分化本身反映结构特征）。
