...
> 右侧「参考资料」列对应 `00_总纲_资料索引.md` 第四部分的知识块编号，每个编号下有完整的中英文资料、必须章节和摘要。

| 阶段 | 天数 | 覆盖知识块 | 参考资料入口 |
|------|------|-----------|-------------|
| 第一阶段：大模型基础认知 | 5 天 | ① 大模型 API · ⑦ 提示词工程 | → `00_总纲_资料索引.md#1-大模型-api编程化调用` · `#7-提示词工程系统化` |
| 第二阶段：RAG 检索增强 | 4 天 | ⑤ RAG + 向量数据库 | → `00_总纲_资料索引.md#5-rag-检索增强--向量数据库` |
| 第三阶段：Agent 核心架构 | 9 天 | ② Agent 机制 · ③ Tools/Skills · ④ MCP · ⑥ LangChain/LangGraph · ⑧ Agent 集群 | → `00_总纲_资料索引.md#2-agent-核心机制` · `#3-tools--skills-体系` · `#4-mcpmodel-context-protocol` · `#6-langchain--langgraph-编排` · `#8-agent-集群路由与跨-agent-记忆` |
| 第四阶段：企业级工程落地 | 6 天 | ⑨ 鉴权 · ⑩ 可观测性 · ⑪ 业务/项目匹配 | → `00_总纲_资料索引.md#9-鉴权体系` · `#10-可观测性` · `#11-业务项目匹配度` |
| 第五阶段：模型微调与私有化 | 6 天 | ⑫ 模型微调 · ⑬ 数据工程 · ⑭ 推理优化 | → `00_总纲_资料索引.md#12-模型微调sft--peft--dpo` · `#13-数据工程` · `#14-推理优化` |
| 第六阶段：多模态 Agent | 5 天 | ⑮ 多模态 Agent | → `00_总纲_资料索引.md#15-多模态-agent` |

### 使用方式

1. **看路线** → 先读本文的周计划表，了解每周目标
2. **查资料** → 根据「参考资料入口」列，跳转到 `00_总纲_资料索引.md` 对应知识块
...
> **实际执行进度（2026-07-30 校准）**：每周比原计划顺延约 2 周。Week 1-4 已完成，Week 4（7/27-31）收尾中，Week 5（8/3-7）下周开始。下方周日期已同步为实际执行日期。

---

## 按周执行计划

### 第一周（Jul 3-10）　知识块 ① ⑦　　API 要能调，Prompt 要能工程化

> **本周一句话**：把模型当成一个函数来调，而不是一个聊天框。

```
Mon  Jul 6  █  大模型谱系 + API 入门      读完 ① 必须章节①②，跑通第一个 API 调用（非流式）
Tue  Jul 7  ██  Function Calling / Tool Use  搞懂 tool 定义 → 模型请求 → 解析结果 → 回传 的循环，跑通 demo
Wed  Jul 8  ██  System Prompt 工程化         三层 Prompt 架构（系统级/场景级/用户级）+ JSON Schema 约束，写到代码里
Thu  Jul 9  █  原理速览 + 本周消化           Token/Context/Temperature 含义，补充落下的代码
```
...
### 第二周（Jul 13-17）　知识块 ⑤ ②　　RAG + Agent 概念，为动手铺路

> **本周一句话**：让模型"知道"你的私有数据，理解 Agent 是怎么做决策的。

```
Mon  Jul 13 █  RAG 基础 + 向量数据库         Embedding 选型（bge-large-zh-v1.5）、分块策略、Milvus 跑起来
Tue  Jul 14 ██  混合检索 + 重排序            BM25 + 向量融合、Cross-Encoder 重排、端到端 RAG 管线
Wed  Jul 15 █  Agent 概念入门               ReAct 模式（Thought→Action→Observation）、工具调用循环、记忆三层
Thu  Jul 16 ██  Anthropic Agent 设计哲学      读"Building Effective Agents"全文，理解什么时候用 Workflow vs Agent
Fri  Jul 17 █  本周 Demo 串联                把 RAG 管线 + Agent 循环拼到一起，Agent 能检索你的文档回答问题
```
...
### 第三周（Jul 20-24）　知识块 ③ ④ ⑥　　开始写 Agent！

> **本周一句话**：这一周开始，你的 Demo 从一个"脚本"变成一个有架构的 Agent 系统。

```
Mon  Jul 20 ██  工具体系设计                工具注册/发现机制、Schema 规范化（name/description/parameters）、工具粒度原则
Tue  Jul 21 ██  MCP Server 开发             跑通一个 MCP Server（Python SDK），把上周的 2 个工具 + 新增工具都注册进去
Wed  Jul 22 ██  MCP Client + 多 Server       编写 Client 端连接管理，接入订单 Server + 客户 Server，工具按来源分组
Thu  Jul 23 ██  LangGraph 状态图             用 StateGraph 重构 Agent 循环（替代手写 while 循环），加条件边实现路由
Fri  Jul 24 █  本周 Demo 串连 + 回顾          MCP 标准化工具 + LangGraph 编排的单 Agent，代码结构清晰、可扩展
```
...
### 第四周（Jul 27-31）　知识块 ⑧ ⑨　　多 Agent 集群 + 鉴权 🎤 7/29 分享！

> **本周一句话**：一个 Agent 不够用，让多个 Agent 协作并管好权限。

```
Mon  Jul 27 ██  多 Agent 协作模式             Supervisor/Manager-Worker 模式实现，主 Agent 拆解任务 → 分发给子 Agent
Tue  Jul 28 ██  Agent 路由 + 准备分享           意图识别路由、结果聚合；晚上打磨 Demo，准备 7/29 分享大纲和跑通核心流程
Wed  Jul 29 🎤  分享日！                        上午过一遍 PPT + 现场跑通，下午正式分享（20min + Q&A）
Thu  Jul 30 ██  鉴权体系                      Token Exchange（RFC 8693）、API Key 管理、用户身份透传、工具级权限（RBAC+ABAC）
Fri  Jul 31 █  多租户隔离 + 本周 Demo 串连      洋葱型防御（网关→运行时→工具三层）、审计日志；多 Agent + 鉴权完整联调
```
...
### 第五周（Aug 3-7）　知识块 ⑩ ⑪　　可观测 + 业务匹配

> **本周一句话**：一个能上线的 Agent 系统，必须有监控、有评估、有业务判断依据。

```
Mon  Aug 3  ██  可观测性                      全链路追踪（Langfuse + OpenTelemetry）、五大指标、Span 三层设计
Tue  Aug 4  ██  成本监控 + 告警                Token 用量追踪、成本三级告警（100%/150%/200%）、预算熔断
Wed  Aug 5  ██  业务匹配度 + 评估              场景适用性判断（Prompt/Workflow/Agent 边界）、ROI 框架、Agent 评估体系
Thu  Aug 6  ██  综合实战                      给 week4 的 Demo 接入可观测 + 评估，补齐生产级最后一块
Fri  Aug 7  █  回顾 + 中期演示                 前 5 周成果 review，准备一个 15min 的 Demo 演示
```
...
### 第六周（Aug 10-14）　知识块 ⑫ ⑬ ⑭　　微调 + 推理优化

> **本周一句话**：知道什么时候 Prompt/RAG 不够用、需要微调，怎么把推理成本降下来。

```
Mon  Aug 10 █  微调决策框架                  Prompt vs RAG vs Fine-tuning 边界线、成本估算、数据量评估
Tue  Aug 11 ██  LoRA/QLoRA 实战              用 PEFT 跑通一个 QLoRA 微调（Qwen2.5-7B），12GB 显存可跑
Wed  Aug 12 █  DPO 偏好对齐                  DPO vs RLHF 选型、偏好对构建、DPO 比 RLHF 简单在哪
Thu  Aug 13 ██  数据工程 + 推理优化            IFD/LESS 数据筛选、vLLM + AWQ + 前缀缓存、KV Cache 调优
Fri  Aug 14 █  本周消化 + 补代码              把微调/推理概念落实到 Demo 的配置和选型文档里
```
...
### 第七周（Aug 17-21）　知识块 ⑮　　多模态 Agent

> **本周一句话**：拓展 Agent 的感知边界——能看图、能听语音、能读 PDF。

```
Mon  Aug 17 ██  视觉 Agent                    GPT-4o/Claude Vision API、截图理解、文档解析（PDF/表格/扫描件）
Tue  Aug 18 ██  语音 Agent                    Whisper ASR + TTS 语音闭环、延迟优化（P99 < 2s）
Wed  Aug 19 █  视频理解 + 模态路由             关键帧提取、统一多模态路由器架构
Thu  Aug 20 ██  把多模态接入现有 Demo           给 week5 系统加视觉/语音能力，保持架构整洁
Fri  Aug 21 █  7 周回顾 + 最终演示              完整路线成果验收，准备 30min 技术分享
```
...
Week 4  █████████  多 Agent 集群 + 鉴权            产出：多 Agent 协作系统 ⭐ 🎤 7/29 分享！
...
> **注**：分享实际是否按期进行待 Week 4 复盘确认（原计划 7/23，随执行顺延至 7/29）。

**Jul 29（第四周周三）做一次技术分享**，此时已完成的内容：
...
> **建议**：分享前利用周末（7/25-26）把 Week 3 的 Demo 打磨干净，代码加 README + 架构图，确保现场能跑。Week 4 周一周二的多 Agent 部分至少做到能口头讲清楚架构思路，即使代码还没完全调通。
...
5. **双文件联动**：本文 = 周计划 + 产出清单，`00_总纲_资料索引.md` 第四部分 = 每个知识块的完整参考资料
